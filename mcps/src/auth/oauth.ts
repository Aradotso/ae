...
<body>
  <div class="card">
    ...
    <p class="subtitle">${client.client_name ?? "An MCP client"} wants to connect to the Ara MCP</p>
    <div class="scope">
      <strong>${client.client_name ?? "This client"}</strong> is requesting access to
      Ara's MCP tools — Railway, Resend, Braintrust, Axiom, Higgsfield,
      Blaxel, Engain, Linq, Postiz, Instacart.
    </div>
...
<body><div class="card">
  <h1>Access Denied</h1>
  <p>${email} is not authorized to use the Ara MCP. Only @ara.so team accounts are allowed.</p>
</div></body></html>`);
      return;
    }
